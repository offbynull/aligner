...
private:
    ...
    std::vector<std::vector<ND>> nodes;
    std::vector<std::vector<edge_data_set>> edges;
...
public:
    grid_graph(T _down_node_cnt, T _right_node_cnt) : down_node_cnt{_down_node_cnt}, right_node_cnt{_right_node_cnt}, nodes{_down_node_cnt}, edges{_down_node_cnt} {
        for (T i = 0u; i < down_node_cnt; ++i) {
            nodes[i] = std::vector<ND>(right_node_cnt);
        }
        for (T i = 0u; i < down_node_cnt; ++i) {
            edges[i] = std::vector<edge_data_set>(right_node_cnt);
        }
    }
    ...
    void update_node_data(const N& node, ND&& data) {
        ...
        return this->nodes[n_down][n_right] = std::forward<ED>(data);
    }
    ...
    ND& get_node_data(const N& node) {
        ...
        auto [n_down, n_right] = node;
        return this->nodes[n_down][n_right];
    }
    ...
    void update_edge_data(const E& edge, ED&& data) {
        ...
        if (n1_down == n2_down && n1_right + 1u == n2_right) {
            this->edges[n1_down][n1_right].right = std::forward<ED>(data);
        } else if (n1_down + 1u == n2_down && n1_right == n2_right) {
            this->edges[n1_down][n1_right].down = std::forward<ED>(data);
        } else if (n1_down + 1u == n2_down && n1_right + 1u == n2_right) {
            this->edges[n1_down][n1_right].diag = std::forward<ED>(data);;
        }
    }
    ...
    ED& get_edge_data(const E& edge) {
        ...
        auto [n2_down, n2_right] = edge.second;
        if (n1_down == n2_down && n1_right + 1u == n2_right) {
            return this->edges[n1_down][n1_right].right;
        } else if (n1_down + 1u == n2_down && n1_right == n2_right) {
            return this->edges[n1_down][n1_right].down;
        } else if (n1_down + 1u == n2_down && n1_right + 1u == n2_right) {
            return this->edges[n1_down][n1_right].diag;
        }
    ...
    auto get_leaf_nodes() {
        return std::ranges::single_view { std::pair<T, T>(this->down_node_cnt - 1u, this->right_node_cnt - 1u) };
    }

    auto get_nodes() {
        auto down_range { std::views::iota(0u, this->down_node_cnt) };
        auto right_range { std::views::iota(0u, this->right_node_cnt) };
        return
    ...
    auto get_outputs_full(const N& node) {
        ...
        if (n_down == down_node_cnt - 1u && n_right == right_node_cnt - 1u) {
            return std::vector<std::tuple<E, N, N, ED&>> {};
        } else if (n_down < down_node_cnt - 1u && n_right < right_node_cnt - 1u) {
            return std::vector<std::tuple<E, N, N, ED&>> {
                this->construct_full_edge(node, N {n_down, n_right + 1u}),
                this->construct_full_edge(node, N {n_down + 1u, n_right}),
                this->construct_full_edge(node, N {n_down + 1u, n_right + 1u})
            };
        ...
        // TODO: MAKE THIS INTO SOMETHING LIKE std::array BECAUSE std::vector DOES HEAP ALLOCS -- USE std::views::take() TO LIMIT THE SIZE OF ARRAY
    }
    ...
    auto get_inputs_full(const N& node) {
        ...
        if (n_down == 0u && n_right == 0u) {
            ...
            return std::vector<std::tuple<E, N, N, ED&>> {
                this->construct_full_edge(N {n_down, n_right - 1u}, node),
                this->construct_full_edge(N {n_down - 1u, n_right}, node),
                this->construct_full_edge(N {n_down - 1u, n_right - 1u}, node)
            };
        } else if (n_right > 0u) {
            return std::vector<std::tuple<E, N, N, ED&>> {
                this->construct_full_edge(N {n_down, n_right - 1u}, node)
