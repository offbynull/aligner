...
void populate_weights_and_backtrack_pointers(
    ...
    for (const auto& node : processed_nodes) {
        for (const auto& e : g.get_outputs(node)) {
            ...
            auto e_from_set {
                g.get_inputs(dst_node)
                | std::views::transform([&g](auto _e) { return g.get_edge_from(_e); })
            };
    ...
    while (waiting_nodes.size() > 0u) {
        ...
        bool is_subset {
            std::includes(processed_nodes.begin(), processed_nodes.end(), incoming_nodes.begin(), incoming_nodes.end())
        };
        if (!is_subset) {
            continue;
        }
