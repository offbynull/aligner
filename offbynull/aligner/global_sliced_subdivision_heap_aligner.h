#ifndef OFFBYNULL_ALIGNER_GLOBAL_SLICED_SUBDIVISION_HEAP_ALIGNER_H
#define OFFBYNULL_ALIGNER_GLOBAL_SLICED_SUBDIVISION_HEAP_ALIGNER_H
...
namespace offbynull::aligner::global_sliced_subdivision_heap_aligner {
    ...
    /**
     * Concept that's satisfied if `T` is has the traits to configure a dynamic programming global alignment to use heap-based memory.
     *
     * @tparam T Type to check.
     */
    template<typename T>
    concept sliced_subdivision_heap_allocation_strategy = requires {
        { T::minimize_allocations } -> std::same_as<const bool &>;  // Preallocate max possible sizes?
        compile_time_constant<T::minimize_allocations> {};
    };

    /**
     * Standard @ref sliced_subdivision_heap_allocation_strategy implementation.
     *
     * @tparam minimize_allocations_ `true` to force certain internal containers to reserve (preallocate) elements thereby
     *     removing/reducing the need for adhoc reallocations, `false` otherwise.
     */
    template<bool minimize_allocations_>
    struct sliced_subdivision_heap_allocation_strategy_config {
        /** Force certain internal containers to reserve (preallocate) elements when `true` (copy of `minimize_allocations_`). */
        static constexpr bool minimize_allocations { minimize_allocations_ };
    };
    ...
    public:
        template<
            sliced_subdivision_heap_allocation_strategy allocation_strategy = sliced_subdivision_heap_allocation_strategy_config<true>,
        ...
        template<
            ...
            weight WEIGHT,
            sliced_subdivision_heap_allocation_strategy allocation_strategy
        >
        auto align_strict(
            const sequence auto& down,
            const sequence auto& right,
            ...
            using CONTAINER_CREATOR_PACK = backtracker_heap_container_creator_pack<
                ...
                typename G::E,
                typename G::ED,
                allocation_strategy::minimize_allocations
            >;
            backtracker<
                debug_mode,
                G,
...
#endif //OFFBYNULL_ALIGNER_GLOBAL_SLICED_SUBDIVISION_HEAP_ALIGNER_H
