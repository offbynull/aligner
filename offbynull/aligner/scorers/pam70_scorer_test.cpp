...
namespace {
    ...
    TEST(Pam70ScorerTest, SanityTest) {
        pam70_scorer<int> scorer {};
        char a_ { 'A' };
        char c_ { 'C' };
        EXPECT_EQ(5, (scorer(std::tuple<>{}, { { a_ } }, { { a_ } })));
        EXPECT_EQ(-4, (scorer(std::tuple<>{}, { { a_ } }, { { c_ } })));
        EXPECT_EQ(-11, (scorer(std::tuple<>{}, { { a_ } }, { std::nullopt })));
        EXPECT_EQ(9, (scorer(std::tuple<>{}, { { c_ } }, { { c_ } })));
        EXPECT_EQ(-4, (scorer(std::tuple<>{}, { { c_ } }, { { a_ } })));
        EXPECT_EQ(-11, (scorer(std::tuple<>{}, { { c_ } }, { std::nullopt })));;
        EXPECT_EQ(-11, (scorer(std::tuple<>{}, { std::nullopt }, { { a_ } })));
        EXPECT_EQ(-11, (scorer(std::tuple<>{}, { std::nullopt }, { { c_ } })));
