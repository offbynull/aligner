...
namespace offbynull::aligner::scorer::scorer {
    ...
    concept scorer =
        ...
        && std::is_same_v<std::decay_t<DOWN_ELEM>, DOWN_ELEM>  // make sure no refs / cv-qualifiers
        && std::is_same_v<std::decay_t<RIGHT_ELEM>, RIGHT_ELEM>  // make sure no refs / cv-qualifiers
        && weight<WEIGHT>;
}
